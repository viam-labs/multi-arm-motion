...
- **UR arm module silently drops trajectories it won't execute (severity: medium).** The `viam:universal-robots` module can accept a `MoveThroughJointPositionsStreamed` call, decide the trajectory is unsafe, and then neither execute it nor return an error — the stream just hangs. The UR module should return an error the caller can act on.
